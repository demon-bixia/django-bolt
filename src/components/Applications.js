...
const Applications = ({appList}) => {
    ...
    return (<Box sx={{marginLeft: theme.spacing(8), display: 'flex', flexDirection: "column"}}>
        <Box className="applications-list" sx={{marginBottom: theme.spacing(3),}}>
            {/* list of apps */}
            {appList.map((app) => (<Box className="application" key={app.name} sx={{marginBottom: theme.spacing(6),}}>
                <Typography variant="h4" sx={{fontWeight: "500", marginBottom: theme.spacing(5),}}>
                    {app.name}
                </Typography>
                {app.models.map((model) => {
                    ...
                    return (<Paper key={model_name} elevation={0} sx={{
                        ...
                        <Link href="#" underline="none" variant="body1"
                              sx={{color: theme.palette.text.secondary, flexGrow: 2,}}>
                            {model.name}
                        </Link>
